...
%% you need to load first stereoParamsO1.mat to your matlab workspace
%

function multiObjectTrackingMod(videoFile,videoRight,stereoParamsO1)

% Create System objects used for reading video, detecting moving objects,
% and displaying the results.
obj = setupSystemObjects();
objR = setupSystemObjectsR();
...
while ~isDone(obj.reader)
    ...
    if x==1
     ...
     displayTrackingResults(frame,frame2,stereoParamsO1,bboxes);
...
%         obj.reader = vision.VideoFileReader('atrium.avi');
 ...
 function objR = setupSystemObjectsR()
        % Initialize Video I/O
        % Create objects for reading a video from a file, drawing the tracked
        % objects in each frame, and playing the video.
        ...
        % Create a video file reader.
%         obj.reader = vision.VideoFileReader('atrium.avi');
        objR.reader=vision.VideoFileReader(videoRight);
        % Create two video players, one to display the video,
        % and one to display the foreground mask.
        objR.videoPlayer = vision.VideoPlayer('Position', [20, 400, 700, 400]);
        objR.maskPlayer = vision.VideoPlayer('Position', [740, 400, 700, 400]);
        ...
        objR.detector = vision.ForegroundDetector('NumGaussians', 3, ...
            'NumTrainingFrames', 40, 'MinimumBackgroundRatio', 0.5, ...
            'LearningRate',.005);
        ...
        objR.blobAnalyser = vision.BlobAnalysis('BoundingBoxOutputPort', true, ...
            'AreaOutputPort', true, 'CentroidOutputPort', true, ...
            'MinimumBlobArea', 800);
    end
%% Initialize Tracks
...
% Read the next video frame from the video file.
 ...
 function frameR = readFrameR()
        frameR = objR.reader.step();
    end
%% Detect Objects
...
%         mask = imopen(mask, strel('rectangle', [1,2]));
%         mask = imclose(mask, strel('rectangle', [15, 15])); 
%         mask = imfill(mask, 'holes');
%         mask = imdilate(mask, strel('rectangle', [4,4]));
...
% displays the frame and the mask in their respective video players. 
    ...
    function displayTrackingResults(frame,frame2,stereoParamsO1,bboxes)
        ...
        if ~isempty(tracks)
            ...
            if ~isempty(reliableTracks) 
                ...
                labels = calculateDepth(frame,frame2,stereoParamsO1,bboxes);
                % Get ids.
...
if ~isempty(labels)
    ...
    function [labels2] = calculateDepth(leftImg, rightImg,stereoParamsO1,bboxes)
        
        %% Get images

% leftImg = imread('\\172.16.102.110\ServerShared\test1204\left\left_session_2015-04-12_09-45-22_10.jpg');
% rightImg = imread('\\172.16.102.110\ServerShared\test1204\right\right_session_2015-04-12_09-45-22_10.jpg');

%% Rectify images and plot disparity map
% imshowpair(leftImg,rightImg,'Montage')
[leftImgRect, rightImgRect] = rectifyStereoImages(leftImg, rightImg, stereoParamsO1);
...
point3D = reconstructScene(dispMap, stereoParamsO1);


z = point3D(:,:,3);
z(z < 0 | z > 8000) = NaN;
x1 = point3D(:,:,1);
x1(x1 < -3000 | x1 > 3000) = NaN;
point3D(:,:,3) = z;
point3D(:,:,1) = x1;


% pointCloud = reconstructScene(dispMap, stereoParamsO1);
...
% bboxes1 = peopleDetector.step(leftImgRect);
...
Y = point3D(:, :, 2);
Z = point3D(:, :, 3);
J = flipud(imresize(leftImgRect,size(X)));
...
labels2 = cell(1, numel(dists));
for h = 1:numel(dists)
    labels2{h} = sprintf('%0.2f Feet', dists(h));
end

else
    
    centroids(:, 2) =abs(centroids(:, 2) );
    centroids(:, 1) =abs(centroids(:, 1) );
